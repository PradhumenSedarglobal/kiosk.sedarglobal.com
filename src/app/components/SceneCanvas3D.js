 ...
 console.log(props, 'props'); 
  ...
  const loadScene = () => {
    ...
    const mesh3d = new THREE.Mesh(
      new THREE.SphereGeometry(480, 60, 40).scale(-1, 1, 1),
      material
    );
  ...
  const onDocumentMouseWheel = (event) => {
    ...
    camera.fov = THREE.MathUtils.clamp(fov, 10, camera_fov + 20);
    camera.updateProjectionMatrix();
    return fov;
  };
  const resetScean = () => {
    camera.fov = THREE.MathUtils.clamp(camera_fov, 10, camera_fov);
    camera.updateProjectionMatrix();
    lat = 0;
    lon = 180;
  };
  const goTocartPage = () => {
    ...
    let cust_type = customization?.user_info
      ? customization?.user_info.cust_type
      : "";
    let SOL_SOH_SYS_ID =
      customization?.modification_user_info &&
  ...
  if (cart_status == "INCOMPLETE") {
    cart_status =
      editStepData.line_result &&
        ["MODIFICATION", "COMPLETED"].indexOf(
  ...
  if (state.user_info && state.user_info.cust_id) {
    userId =
      editStepData.line_result &&
        state.user_info &&
        state.user_info &&
        state.user_info.cust_type == "ADMIN"
        ? editStepData.line_result.SOL_CUST_SYS_ID
        : state.user_info?.cust_id;
    modify_cust_sys_id =
      state.user_info && state.user_info && state.user_info.cust_type == "ADMIN"
        ? state.user_info.cust_id
        : 0;
    SOL_SOH_SYS_ID =
      editStepData.line_result && editStepData.line_result.SOL_SOH_SYS_ID > 0
        ? editStepData.line_result.SOL_SOH_SYS_ID
        : "";
    SOL_CAD_SYS_ID =
      editStepData.line_result && editStepData.line_result.SOL_CAD_SYS_ID > 0
        ? editStepData.line_result.SOL_CAD_SYS_ID
        : "";
  }
  let url =
